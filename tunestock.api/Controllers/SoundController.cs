...
public class SoundController : ControllerBase
{
    ...
    public async Task<ActionResult<Response<List<SoundDto>>>> GetByEmail(int user_id)
    {
        var response = new Response<List<SoundDto>>();

        try
        {
            response.Data = await _soundService.GetByUser(user_id);
            return Ok(response);
        }
        catch (Exception ex)
        {
            Console.WriteLine("HA OCURRIDO UN ERROR - SoundController (GetByUser): " + ex.Message);
            response.Errors.Add("HA OCURRIDO UN ERROR - SoundController (GetByUser)");
            return StatusCode(500, response);
        }
    }
    ...
    [HttpPost]
    public async Task<ActionResult<Response<SoundDto>>> Post([FromBody] InputSoundDto inputSoundDto,
