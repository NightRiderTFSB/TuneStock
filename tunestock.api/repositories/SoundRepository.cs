...
public class SoundRepository : ISoundRepository
{
    ...
    public async Task<List<Sound>> GetByUser(int ID) {
        try
        {
            string query = $"SELECT * FROM Sound WHERE IsDeleted = 0 AND UserID = {ID}";
            var sounds = await _dbContext.Connection.QueryAsync<Sound>(query);
            Console.WriteLine("OBTENIDOS CON EXITO - SoundRepository (GetByUser)");
    ...
    public async Task<Sound> GetByID(int SoundID)
    {
        try
        {
            ...
            return sound.IsDeleted ? null : sound;
        }
        catch (Exception ex)
        {
    ...
    public async Task<Sound> SaveAsync(Sound sound, int labelId)
    {
        try
        {
            ...
            await _dbContext.Connection.ExecuteAsync("InsertSoundAndLabel", parameters,
